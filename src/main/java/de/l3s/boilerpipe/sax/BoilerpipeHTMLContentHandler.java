...
public class BoilerpipeHTMLContentHandler implements ContentHandler {
	...
	public void flushBlock() {
		...
		final Collection<CharSequence> tokens = UnicodeTokenizer.tokenize(tokenBuffer);
		...
		for (final CharSequence token : tokens) {
			if (ANCHOR_TEXT_START.contentEquals(token)) {
				inAnchorText = true;
			} else if (ANCHOR_TEXT_END.contentEquals(token)) {
				inAnchorText = false;
	...
	private static boolean isWord(final CharSequence token) {
